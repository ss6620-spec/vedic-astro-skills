...
支持 romantic / business / friendship。
**两段式入口（先盲扫性质，再进框架），intake 放在平扫之后：**

```
Layer 0.5（关系性质盲扫，不预设类型，无需 intake）
        ↓ ⏸️ 暂停 → 输出性质指纹 → intake：问诉求 + 关系模式（默认情感）
Layer 0-3（进入用户选定的关系框架深析）
```

为什么 intake 放后面：先用平扫的性质指纹帮用户判断这关系是什么底色，再决定要不要深入、进哪个框架（如平扫是"事业互补"，可能选 business 而非默认情感）。

支持两种用法：
- **只平扫**：用户只想了解"两人最突出的连接性质" → 跑完 Layer 0.5 即停，不写 01–04。
- **完整深析**：Layer 0.5 → intake 选定框架（默认 romantic）→ Layer 0–3。
...
### ⏸️ 平扫后暂停（必停，第三种流程核心）
输出性质指纹后停下，向用户 intake：
> "这两个人最突出的连接是 [性质指纹]（如：业力纠缠 + 高吸引高消耗）。
>  要进入完整分析吗？想看哪种关系——情感(默认) / 合作 / 友谊？有具体想问的吗？"

- 用户只要平扫结果 → 到此结束，不写 01–04。
- 用户要深入 / 直接说继续 → 把关系模式+诉求记入 intake.md，按选定框架（默认情感）继续 Layer 0。
- 用户无回应但需推进 → 默认情感模式继续。
