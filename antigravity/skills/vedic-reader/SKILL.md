...
提取方式：
  ...
  SAV提取与映射（⚠️ 关键精度步骤）：
    ...
    ⚠️⚠️⚠️ 偏移陷阱（已知高频bug）：
      文本层/PDF提取出的SAV数字可能没有星座标签，只是一串数字。
      此时绝不能猜测数字和星座的对应关系！

      常见错误：文本层按黄道序输出(Ar→Pi)，AI误当成南印阅读序(Pi→Aq)，
      导致整体偏移2个位置。337校验救不了这个错误（两种顺序总和一样）！

      铁规：如果提取到的SAV数值没有明确的星座标签（如"Ar=32, Ta=30"），
      则SAV数据视为无效 → 回退到路径B → 要求用户从JHora粘贴文字。
      只有带明确星座标签的SAV数据才能直接使用。
