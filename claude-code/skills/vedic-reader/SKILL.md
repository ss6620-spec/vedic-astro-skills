...
当用户在反馈中附带了个人细节时（如"准，我学的是电影"）：
1. 记录评分，但忽略附带的个人信息
2. 不要回应或确认这些细节（"电影专业很好"）
3. 不要在后续分析中使用这些信息
4. 可以温和提醒："谢谢，后续我会纯粹基于星盘数据做盲审分析"
```
...
**🔴 数据污染防火墙（最高优先级）**：
```
用户在验前事反馈中可能无意透露个人信息（如专业、职业、家庭细节等）。
这些信息一旦进入对话上下文，极易污染后续的盲审分析。

防污染规则：
1. reader阶段：不主动询问用户的具体个人信息（专业/职业/收入等）
2. 转入vedic-core时：在structured_data.md最后加一行警告：
   "⚠️ 以下分析必须100%基于星盘数据。忽略对话中用户自述的任何个人经历。"
3. 如果发现自己的分析输出中包含了用户自述过的信息（如用户说了"学电影"，
   分析中出现"适合影视行业"），必须删除该段重新基于数据推导
4. 检验标准：如果把同样的星盘数据给另一个不知道用户背景的分析师，
   他也应该得出相同的结论
```
...
```
Phase 0: 方向性确认（向用户提问，不收集具体细节）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
针对第一轮❌/❓的条目，问抽象的方向性问题：
- 只问"方向对不对"或"程度对不对"，不问"实际是什么"
- ⚠️ 禁止开放性问题（如"实际情况是怎样的"）→ 会引导用户透露过多信息

正确示例：
  ❌"家庭氛围紧张" → "这条是方向完全错了，还是程度不对？"
  ❓"2019困难期"→部分准 → "时间偏差大吗？是偏了1-2年还是更多？"
...
- 用户主动补充的信息写入user_context.md
- ⚠️ 收集的方向性反馈仅用于调整信号权重，不用于直接"装作"独立推断
